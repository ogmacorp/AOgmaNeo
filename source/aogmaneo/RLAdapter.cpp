...
void RLAdapter::learn(
    ...
    for (int ix = iterLowerBound.x; ix <= iterUpperBound.x; ix++)
        for (int iy = iterLowerBound.y; iy <= iterUpperBound.y; iy++) {
            ...
            int inCIPrev = history[t].hiddenCIs[otherHiddenColumnIndex];

            Int2 offset(ix - fieldLowerBound.x, iy - fieldLowerBound.y);

            int wi = inCIPrev + hiddenSize.z * (offset.y + diam * (offset.x + diam * hiddenCellIndexTarget));
    ...
    float delta = lr * (history[t - 1].reward + discount * maxActivation - sumPrev);
