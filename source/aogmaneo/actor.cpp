    ...
    for (int vli = 0; vli < visible_layers.size(); vli++) {
        ...
        for (int ix = iter_lower_bound.x; ix <= iter_upper_bound.x; ix++)
            for (int iy = iter_lower_bound.y; iy <= iter_upper_bound.y; iy++) {
                ...
                vl.value_weights[wi_offset + hidden_column_index * hidden_stride] += delta_value * vl.gates[visible_column_index];
            }
    }
        ...
        for (int ix = iter_lower_bound.x; ix <= iter_upper_bound.x; ix++)
            for (int iy = iter_lower_bound.y; iy <= iter_upper_bound.y; iy++) {
                ...
                for (int hc = 0; hc < hidden_size.z; hc++) {
                    ...
                    float delta_action = rate * ((hc == target_ci) - hidden_acts[hidden_cell_index]) * vl.gates[visible_column_index];
...
void Actor::init_random(
    ...
    for (int vli = 0; vli < visible_layers.size(); vli++) {
        ...
        vl.usages = Byte_Buffer(vl.action_weights.size(), 0);
...
int Actor::size() const {
    ...
    for (int vli = 0; vli < visible_layers.size(); vli++) {
        ...
        size += sizeof(Visible_Layer_Desc) + vl.value_weights.size() * sizeof(float) + 2 * vl.action_weights.size() * sizeof(Byte);
    }
...
void Actor::read(
    ...
    for (int vli = 0; vli < visible_layers.size(); vli++) {
        ...
        vl.value_weights.resize(num_hidden_columns * area * vld.size.z);
        vl.action_weights.resize(num_hidden_cells * area * vld.size.z);
        vl.usages.resize(vl.action_weights.size());
