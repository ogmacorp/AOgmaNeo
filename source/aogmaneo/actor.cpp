                    ...
                    for (int vc = 0; vc < vld.size.z; vc++) {
                        ...
                        for (int hc = 0; hc < hidden_size.z; hc++) {
                            ...
                            for (int di = 0; di < policy_num_dendrites_per_cell; di++) {
                                ...
                                if (vc == in_ci_prev) {
                                    ...
                                    vl.policy_traces[wi] += error * ((di >= half_policy_num_dendrites_per_cell) * 2.0f - 1.0f) * ((policy_dendrite_acts_prev[dendrite_index] > 0.0f) * (1.0f - params.leak) + params.leak);// * expf(-abs(trace) * params.trace_curve);
                                }

                                vl.policy_weights[wi] += policy_error_partial * min(1.0f, max(-1.0f, vl.policy_traces[wi] * params.trace_curve));
                            }
                        }
                        ...
                        for (int di = 0; di < value_num_dendrites_per_cell; di++) {
                            ...
                            if (vc == in_ci_prev) {
                                ...
                                vl.value_traces[wi] += ((di >= half_value_num_dendrites_per_cell) * 2.0f - 1.0f) * ((value_dendrite_acts_prev[dendrite_index] > 0.0f) * (1.0f - params.leak) + params.leak);// * expf(-abs(trace) * params.trace_curve);
                            }

                            vl.value_weights[wi] += value_delta * min(1.0f, max(-1.0f, vl.value_traces[wi] * params.trace_curve));
                        }
                    }
                }
        }
...
long Actor::size() const {
    long size = sizeof(Int3) + 2 * sizeof(int) + hidden_cis.size() * sizeof(int) + hidden_values.size() * sizeof(float) + hidden_acts_prev.size() * sizeof(float) + policy_dendrite_acts_prev.size() * sizeof(float) + value_dendrite_acts_prev.size() * sizeof(float) + sizeof(int);
...
void Actor::write(
    ...
    writer.write(reinterpret_cast<const void*>(&policy_num_dendrites_per_cell), sizeof(int));
    writer.write(reinterpret_cast<const void*>(&value_num_dendrites_per_cell), sizeof(int));
...
void Actor::read(
    ...
    reader.read(reinterpret_cast<void*>(&policy_num_dendrites_per_cell), sizeof(int));
    reader.read(reinterpret_cast<void*>(&value_num_dendrites_per_cell), sizeof(int));
