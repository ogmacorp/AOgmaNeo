...
void Actor::forward(
    ...
    if (learn_enabled) {
        ...
        float policy_delta = params.plr * (1.0f - mimic) * scaled_td_error;
                        ...
                        for (int hc = 0; hc < hidden_size.z; hc++) {
                            ...
                            for (int di = 0; di < policy_num_dendrites_per_cell; di++) {
                                ...
                                if (vc == in_ci_prev) {
                                    ...
                                    vl.policy_weights[wi] += params.plr * mimic * policy_dendrite_acts_prev[dendrite_index];
                                }

                                vl.policy_weights[wi] += policy_delta * vl.policy_traces[wi];
...
long Actor::size() const {
    long size = sizeof(Int3) + 2 * sizeof(int) + hidden_cis.size() * sizeof(int) + hidden_values.size() * sizeof(float) + hidden_acts_prev.size() * sizeof(float) + value_dendrite_acts_prev.size() * sizeof(float) + policy_dendrite_acts_prev.size() * sizeof(float) + hidden_td_scales.size() * sizeof(float) + sizeof(int);
