                    ...
                    for (int vc = 0; vc < vld.size.z; vc++) {
                        ...
                        for (int hc = 0; hc < hidden_size.z; hc++) {
                            ...
                            for (int di = 0; di < policy_num_dendrites_per_cell; di++) {
                                ...
                                if (vc == in_ci_prev)
                                    vl.policy_traces[wi] += error * ((di >= half_policy_num_dendrites_per_cell) * 2.0f - 1.0f) * ((policy_dendrite_acts_prev[dendrite_index] > 0.0f) * (1.0f - params.leak) + params.leak) * expf(-params.trace_scale * abs(vl.policy_traces[wi]));
                        ...
                        for (int di = 0; di < value_num_dendrites_per_cell; di++) {
                            ...
                            if (vc == in_ci_prev)
                                vl.value_traces[wi] += ((di >= half_value_num_dendrites_per_cell) * 2.0f - 1.0f) * ((value_dendrite_acts_prev[dendrite_index] > 0.0f) * (1.0f - params.leak) + params.leak) * expf(-params.trace_scale * abs(vl.value_traces[wi]));
