...
void Actor::forward(
    ...
    for (int di = 0; di < value_num_dendrites_per_cell; di++) {
        ...
        value_dendrite_acts[dendrite_index] = sigmoidf(act);

        value_diff += softplusf(act) * ((di >= half_value_num_dendrites_per_cell) * 2.0f - 1.0f);
    }
    ...
    for (int hc = 0; hc < hidden_size.z; hc++) {
        ...
        for (int di = 0; di < policy_num_dendrites_per_cell; di++) {
            ...
            policy_dendrite_acts[dendrite_index] = sigmoidf(act);
            ...
            for (int ix = iter_lower_bound.x; ix <= iter_upper_bound.x; ix++)
                for (int iy = iter_lower_bound.y; iy <= iter_upper_bound.y; iy++) {
                    ...
                    for (int vc = 0; vc < vld.size.z; vc++) {
                        ...
                        if (vc == in_ci_prev)
                            vl.value_traces_base[wi_value_base] += 1.0f; // accumulating trace
                        ...
                        for (int hc = 0; hc < hidden_size.z; hc++) {
                            ...
                            for (int di = 0; di < policy_num_dendrites_per_cell; di++) {
                                ...
                                vl.policy_weights[wi] += policy_reinforcement * vl.policy_traces[wi]; // apply deferred sign for dendrites
