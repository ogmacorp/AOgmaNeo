...
void Actor::learn(
    ...
    float td_error_policy = new_value - history_samples[t].hidden_values[hidden_column_index];
    
    float value_delta = params.vlr * td_error_value;

    float policy_error_partial = params.plr * (mimic + (1.0f - mimic) * (td_error_policy > 0.0f ? 1.0f : -(1.0f - params.bias)));
