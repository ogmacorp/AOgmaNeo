        ...
        for (int ix = iter_lower_bound.x; ix <= iter_upper_bound.x; ix++)
            for (int iy = iter_lower_bound.y; iy <= iter_upper_bound.y; iy++) {
                ...
                for (int hc = 0; hc < hidden_size.z; hc++) {
                    ...
                    hidden_acts[hidden_cell_index] += vl.weights[wi] * in_act;
                }
            }
    }
...
void Actor::learn(
    ...
    for (int vli = 0; vli < visible_layers.size(); vli++) {
        ...
        Float_Buffer_View vl_input_acts = history_samples[t - params.n_steps].input_acts[vli];
    ...
    for (int hc = 0; hc < hidden_size.z; hc++) {
        ...
        max_activation_next = max(max_activation_next, hidden_acts[hidden_cell_index]);
    ...
    for (int vli = 0; vli < visible_layers.size(); vli++) {
        ...
        Float_Buffer_View vl_input_acts = history_samples[t].input_acts[vli];
    ...
    //float dPAL = max(dAL, dQ - params.gap * (max_activation_next - next_q_last_action));
    ...
    for (int hc = 0; hc < hidden_size.z; hc++) {
        ...
        // re-use hidden acts as delteas
        hidden_acts[hidden_cell_index] = params.lr * (params.cons * ((hc == target_ci) - hidden_acts[hidden_cell_index]) + (hc == target_ci) * dAL);
    }
        ...
        for (int ix = iter_lower_bound.x; ix <= iter_upper_bound.x; ix++)
            for (int iy = iter_lower_bound.y; iy <= iter_upper_bound.y; iy++) {
                ...
                for (int hc = 0; hc < hidden_size.z; hc++) {
                    ...
                    vl.weights[wi] += hidden_acts[hidden_cell_index] * in_act;
                }
            }
    }
}
...
int Actor::size() const {
    ...
    for (int vli = 0; vli < visible_layers.size(); vli++)
        sample_size += s.input_cis[vli].size() * sizeof(int) + s.input_acts.size() * sizeof(float);
