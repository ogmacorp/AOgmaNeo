...
void Actor::update_gates(
    ...
    int in_ci_prev = history_samples[t].input_cis[vli][visible_column_index];
    ...
    for (int ix = iter_lower_bound.x; ix <= iter_upper_bound.x; ix++)
        for (int iy = iter_lower_bound.y; iy <= iter_upper_bound.y; iy++) {
            ...
            if (in_bounds(column_pos, Int2(visible_center.x - vld.radius, visible_center.y - vld.radius), Int2(visible_center.x + vld.radius + 1, visible_center.y + vld.radius + 1))) {
                ...
                sum_value += vl.value_usages[in_ci_prev + vld.size.z * (offset.y + diam * (offset.x + diam * hidden_column_index))];

                for (int hc =  0; hc < hidden_size.z; hc++) {
                    int hidden_cell_index = hc + hidden_cells_start;

                    sum_action += vl.action_usages[in_ci_prev + vld.size.z * (offset.y + diam * (offset.x + diam * hidden_cell_index))];
                }
    ...
    vl.value_gates[visible_column_index] = expf(-(sum_value / 255.0f) / max(1, count) * params.value_gcurve);
    vl.action_gates[visible_column_index] = expf(-(sum_action / 255.0f) / (max(1, count) * hidden_size.z) * params.action_gcurve);
}
    ...
    for (int vli = 0; vli < visible_layers.size(); vli++) {
        ...
        for (int ix = iter_lower_bound.x; ix <= iter_upper_bound.x; ix++)
            for (int iy = iter_lower_bound.y; iy <= iter_upper_bound.y; iy++) {
                ...
                vl.value_weights[wi_offset + hidden_column_index * hidden_stride] += delta_value * vl.value_gates[visible_column_index];
            }
    }
        ...
        for (int ix = iter_lower_bound.x; ix <= iter_upper_bound.x; ix++)
            for (int iy = iter_lower_bound.y; iy <= iter_upper_bound.y; iy++) {
                ...
                for (int hc = 0; hc < hidden_size.z; hc++) {
                    ...
                    vl.action_weights[wi] += delta_action * vl.action_gates[visible_column_index];
                }
            }
    }
}

void Actor::update_usages(
    const Int2 &column_pos
) {
    int hidden_column_index = address2(column_pos, Int2(hidden_size.x, hidden_size.y));
    ...
    int hidden_cell_index_target = history_samples[0].hidden_target_cis_prev[hidden_column_index] + hidden_cells_start;
    ...
    for (int vli = 0; vli < visible_layers.size(); vli++) {
        ...
        for (int ix = iter_lower_bound.x; ix <= iter_upper_bound.x; ix++)
            for (int iy = iter_lower_bound.y; iy <= iter_upper_bound.y; iy++) {
                ...
                int in_ci = history_samples[1].input_cis[vli][visible_column_index];

                Int2 offset(ix - field_lower_bound.x, iy - field_lower_bound.y);

                {
                    int wi = in_ci + vld.size.z * (offset.y + diam * (offset.x + diam * hidden_column_index));

                    vl.value_usages[wi] = min(255, vl.value_usages[wi] + 1);
                }

                {
                    int wi = in_ci + vld.size.z * (offset.y + diam * (offset.x + diam * hidden_cell_index_target));

                    vl.action_usages[wi] = min(255, vl.action_usages[wi] + 1);
                }
            }
    }
}
...
void Actor::init_random(
    ...
    for (int vli = 0; vli < visible_layers.size(); vli++) {
        ...
        vl.value_usages = Byte_Buffer(vl.value_weights.size(), 0);
        vl.action_usages = Byte_Buffer(vl.action_weights.size(), 0);

        vl.value_gates.resize(num_visible_columns);
        vl.action_gates.resize(num_visible_columns);
    }
...
void Actor::step(
    ...
    if (learn_enabled && history_size > params.min_steps) {
        ...
        PARALLEL_FOR
        for (int i = 0; i < num_hidden_columns; i++)
            update_usages(Int2(i / hidden_size.y, i % hidden_size.y));
    }
}
...
int Actor::size() const {
    ...
    for (int vli = 0; vli < visible_layers.size(); vli++) {
        ...
        size += sizeof(Visible_Layer_Desc) + vl.value_weights.size() * sizeof(float) + vl.action_weights.size() * sizeof(float) + vl.value_usages.size() * sizeof(Byte) + vl.action_usages.size() * sizeof(Byte);
    }
...
void Actor::write(
    ...
    for (int vli = 0; vli < visible_layers.size(); vli++) {
        ...
        writer.write(reinterpret_cast<const void*>(&vl.value_usages[0]), vl.value_usages.size() * sizeof(Byte));
        writer.write(reinterpret_cast<const void*>(&vl.action_usages[0]), vl.action_usages.size() * sizeof(Byte));
    }
...
void Actor::read(
    ...
    for (int vli = 0; vli < visible_layers.size(); vli++) {
        ...
        vl.value_usages.resize(vl.value_weights.size());
        vl.action_usages.resize(vl.action_weights.size());

        reader.read(reinterpret_cast<void*>(&vl.value_usages[0]), vl.value_usages.size() * sizeof(Byte));
        reader.read(reinterpret_cast<void*>(&vl.action_usages[0]), vl.action_usages.size() * sizeof(Byte));
