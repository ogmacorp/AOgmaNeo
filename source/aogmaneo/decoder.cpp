...
void Decoder::forward(
    ...
    for (int hc = 0; hc < hidden_size.z; hc++) {
        ...
        for (int di = 0; di < num_dendrites_per_cell; di++) {
            ...
            if ((!hidden_committed_flags[dendrite_index] || match >= params.vigilance) && activation > max_activation) {
...
void Decoder::learn(
    ...
    float rate = (hidden_committed_flags[dendrite_index_target] ? params.lr : 1.0f);
...
void Decoder::init_random(
    ...
    hidden_committed_flags = Byte_Buffer(num_dendrites, false);
}
...
long Decoder::size() const {
    long size = sizeof(Int3) + sizeof(int) + hidden_cis.size() * sizeof(int) + hidden_dis.size() * sizeof(int) + hidden_committed_flags.size() * sizeof(Byte) + sizeof(int);
...
void Decoder::read(
    ...
    hidden_committed_flags.resize(num_dendrites);
