...
void Decoder::learn(
    ...
    for (int vli = 0; vli < visible_layers.size(); vli++) {
        ...
        for (int ix = iter_lower_bound.x; ix <= iter_upper_bound.x; ix++)
            for (int iy = iter_lower_bound.y; iy <= iter_upper_bound.y; iy++) {
                ...
                int in_ci = vl.recon_cis[visible_column_index];
    ...
    int target_ci = rand_cis[hidden_column_index];
        ...
        for (int ix = iter_lower_bound.x; ix <= iter_upper_bound.x; ix++)
            for (int iy = iter_lower_bound.y; iy <= iter_upper_bound.y; iy++) {
                ...
                if (in_bounds(column_pos, Int2(visible_center.x - vld.radius, visible_center.y - vld.radius), Int2(visible_center.x + vld.radius + 1, visible_center.y + vld.radius + 1))) {
                    int hidden_cell_index = rand_cis[hidden_column_index] + hidden_column_index * hidden_size.z;

                    Int2 offset(column_pos.x - visible_center.x + vld.radius, column_pos.y - visible_center.y + vld.radius);

                    int wi = rand_cis[hidden_column_index] + hidden_size.z * (offset.y + diam * (offset.x + diam * (vc + vld.size.z * hidden_column_index)));
...
void Decoder::step(
    ...
    if (learn_enabled) {
        // rehearsal
        for (int it = 0; it < params.rehearsal_iters; it++) {
            ...
            for (int i = 0; i < num_hidden_columns; i++) {
                ...
                learn(Int2(i / hidden_size.y, i % hidden_size.y), &state, params);
            }
        }
    }
    ...
    for (int i = 0; i < num_hidden_columns; i++) {
        ...
        forward(Int2(i / hidden_size.y, i % hidden_size.y), input_cis, hidden_target_cis, learn_enabled, &state, params);
    }
