...
void Decoder::learn(
    ...
    int di_target = (hidden_acts[hidden_cell_index_target] >= params.vigilance ? hidden_dis[hidden_cell_index_target] : rand(state) % num_dendrites_per_cell);
    int di_max = hidden_dis[hidden_cell_index_max];
        ...
        for (int ix = iter_lower_bound.x; ix <= iter_upper_bound.x; ix++)
            for (int iy = iter_lower_bound.y; iy <= iter_upper_bound.y; iy++) {
                ...
                if (randf(state) < params.forget) {
                    int wi = di_max + num_dendrites_per_cell * (hidden_ci + wi_start);
                ...
                // learn
                {
                    int wi = di_target + num_dendrites_per_cell * (target_ci + wi_start);
