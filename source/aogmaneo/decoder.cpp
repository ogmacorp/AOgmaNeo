            ...
            for (int iy = iter_lower_bound.y; iy <= iter_upper_bound.y; iy++) {
                ...
                for (int hc = 0; hc < hidden_size.z; hc++) {
                    ...
                    for (int di = 0; di < num_dendrites_per_cell; di++) {
                        ...
                        vl.dendrite_sums[dendrite_index] += ((vl.weights[byi] & (1 << bi)) != 0);
                    }
                }
            }
    }
        ...
        for (int ix = iter_lower_bound.x; ix <= iter_upper_bound.x; ix++)
            for (int iy = iter_lower_bound.y; iy <= iter_upper_bound.y; iy++) {
...

                {
                    ...
                    Byte w_old = ((vl.weights[byi] & (1 << bi)) != 0);
                ...
                if (hidden_ci != target_ci && randf(state) < params.fr) {
                    ...
                    vl.dendrite_totals[dendrite_index_max] -= w_old;
                }
            }
    }
}
...
void Decoder::read(
    ...
    for (int vli = 0; vli < visible_layers.size(); vli++) {
        ...
        vl.weights.resize((num_dendrites * area * vld.size.z + 7) / 8);
