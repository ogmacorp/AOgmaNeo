...
void Decoder::forward(
    ...
    for (int hc = 0; hc < hidden_size.z; hc++) {
        ...
        hidden_sums[hidden_cell_index] /= count * 255;
...
void Decoder::learn(
    ...
    float rate = (hidden_totals[hidden_cell_index_target] == 0.0f ? 1.0f : params.lr);
...
void Decoder::init_random(
    ...
    hidden_sums = Float_Buffer(num_hidden_cells, 0.0f);
...
int Decoder::size() const {
    int size = sizeof(Int3) + hidden_cis.size() * sizeof(int) + hidden_sums.size() * sizeof(float) + hidden_totals.size() * sizeof(float) + sizeof(int);
...
int Decoder::state_size() const {
    int size = hidden_cis.size() * sizeof(int) + hidden_sums.size() * sizeof(float);
...
void Decoder::write(
    ...
    writer.write(reinterpret_cast<const void*>(&hidden_sums[0]), hidden_sums.size() * sizeof(float));
...
void Decoder::read(
    ...
    reader.read(reinterpret_cast<void*>(&hidden_sums[0]), hidden_sums.size() * sizeof(float));
