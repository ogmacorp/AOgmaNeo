...
void SparseCoder::forward(
    ...
    if (learnEnabled) {
        ...
        float delta = max(0.0f, abs(error) - threshold) * (error > 0.0f ? alpha : -alpha);

        for (int hc = 0; hc < hiddenSize.z; hc++) {
            int hiddenCellIndex = address3(Int3(columnPos.x, columnPos.y, hc), hiddenSize);

            float cellDelta = delta * ((hc == hiddenCIs[hiddenColumnIndex] ? 1.0f : 0.0f) - hiddenActivations[hiddenCellIndex]);
            ...
            for (int vli = 0; vli < visibleLayers.size(); vli++) {
                ...
                for (int ix = iterLowerBound.x; ix <= iterUpperBound.x; ix++)
                    for (int iy = iterLowerBound.y; iy <= iterUpperBound.y; iy++) {
                        ...
                        vl.weights[inCIPrev + vld.size.z * (offset.y + diam * (offset.x + diam * hiddenCellIndex))] += cellDelta;
                    }
            }
        }
    }
...
void SparseCoder::write(
    ...
    writer.write(reinterpret_cast<const void*>(&threshold), sizeof(float));
...
void SparseCoder::read(
    ...
    reader.read(reinterpret_cast<void*>(&threshold), sizeof(float));
