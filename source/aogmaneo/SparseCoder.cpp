            ...
            for (int ix = iterLowerBound.x; ix <= iterUpperBound.x; ix++)
                for (int iy = iterLowerBound.y; iy <= iterUpperBound.y; iy++) {
                    ...
                    for (int vc = 0; vc < vld.size.z; vc++) {
                        ...
                        vl.weights[wi] = roundftoi(vl.weights[wi] + alpha * ((vc == inCI ? 255.0f : 0.0f) - static_cast<float>(vl.weights[wi])));
                    }
                }
        }
    }
...
int SparseCoder::size() const {
    ...
    for (int vli = 0; vli < visibleLayers.size(); vli++) {
        ...
        size += sizeof(VisibleLayerDesc) + sizeof(int) + vl.weights.size() * sizeof(unsigned char) + vl.inputCIsPrev.size() * sizeof(int);
    }
