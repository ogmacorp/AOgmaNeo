...
void SparseCoder::forward(
    ...
    if (learnEnabled) {
        for (int hc = 0; hc < hiddenSize.z; hc++) {
            ...
            float delta = alpha * tanh((*hiddenErrors)[hiddenColumnIndex]) * ((hc == hiddenCIs[hiddenColumnIndex] ? 1.0f : 0.0f) - hiddenActivations[hiddenColumnIndex]);
    ...
    for (int hc = 0; hc < hiddenSize.z; hc++) {
        ...
        hiddenActivations[hiddenCellIndex] = activation;
...
int SparseCoder::size() const {
    int size = sizeof(Int3) + 2 * sizeof(float) + 2 * hiddenBiases.size() * sizeof(float) + hiddenCIs.size() * sizeof(int) + sizeof(int);
