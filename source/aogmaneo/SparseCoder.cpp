...
void SparseCoder::forward(
    ...
    for (int vli = 0; vli < visibleLayers.size(); vli++) {
        ...
        float scale = 1.0f / max(1, subCount);
...
void SparseCoder::learn(
    ...
    int clumpIndex = address2(clumpPos, clumpCounts);

    // Find highest activation in clump
    int hiddenColumnIndexMax = -1;
    float maxActivation = -999999.0f;

    for (int cx = 0; cx < clumpSize.x; cx++)
        for (int cy = 0; cy < clumpSize.y; cy++) {
            Int2 columnPos(clumpPos.x * clumpSize.x + cx, clumpPos.y * clumpSize.y + cy);

            if (inBounds0(columnPos, Int2(hiddenSize.x, hiddenSize.y))) {
                int hiddenColumnIndex = address2(columnPos, Int2(hiddenSize.x, hiddenSize.y));

                float activation = hiddenActivations[hiddenColumnIndex * hiddenSize.z + hiddenCIs[hiddenColumnIndex]];

                if (activation > maxActivation) {
                    maxActivation = activation;
                    hiddenColumnIndexMax = hiddenColumnIndex;
                }
            }
        }

    assert(hiddenColumnIndexMax != -1);

    Int2 columnPosMax(hiddenColumnIndexMax / hiddenSize.y, hiddenColumnIndexMax % hiddenSize.y);

    int hiddenCellIndexMax = hiddenColumnIndexMax * hiddenSize.z + hiddenCIs[hiddenColumnIndexMax];
    ...
    for (int vli = 0; vli < visibleLayers.size(); vli++) {
        ...
        Int2 visibleCenter = project(columnPosMax, hToV);
...
void SparseCoder::initRandom(
    ...
    clumpCounts = Int2(hiddenSize.x / clumpSize.x, hiddenSize.y / clumpSize.y);
...
void SparseCoder::step(
    ...
    if (learnEnabled) {
        int numClumps = clumpCounts.x * clumpCounts.y;

        #pragma omp parallel for
        for (int i = 0; i < numClumps; i++)
            learn(Int2(i / clumpCounts.y, i % clumpCounts.y), inputCIs);
    }
}

int SparseCoder::size() const {
    int size = sizeof(Int3) + sizeof(Int2) + sizeof(float) + hiddenCIs.size() * sizeof(int) + hiddenVisits.size() * sizeof(int) + sizeof(int);
