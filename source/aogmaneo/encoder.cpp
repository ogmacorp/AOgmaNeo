...
void Encoder::learn_recurrent(
    ...
    for (int tc = 0; tc < temporal_activity; tc++) {
        ...
        // re-use sums as deltas
        recurrent_recon_sums[other_hidden_cell_index] = rand_roundf(params.lr * 255.0f * (((tc + other_spatial_ci * temporal_activity) == target_ci) - (1.0f - expf(-recon_sum * recon_scale))), state);
    }
...
void Encoder::init_random(
    ...
    for (int vli = 0; vli < visible_layers.size(); vli++) {
        ...
        for (int i = 0; i < vl.weights.size(); i++)
            vl.weights[i] = 255 - (rand() % init_weight_noisei_small);
    ...
    for (int i = 0; i < recurrent_weights.size(); i++)
        recurrent_weights[i] = (rand() % init_weight_noisei_small);
