...
void Encoder::learn(
    ...
    for (int vc = 0; vc < vld.size.z; vc++) {
        ...
        vl.recon_deltas[visible_cell_index] = rand_roundf(params.lr * 255.0f * ((vc == target_ci) - expf(min(0.0f, (static_cast<float>(recon_sum) / max(1, count * 255) - 0.5f) * 2.0f * params.scale))), state);
    }
