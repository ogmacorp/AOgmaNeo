...
void Encoder::backward(
    ...
    for (int ix = iter_lower_bound.x; ix <= iter_upper_bound.x; ix++)
        for (int iy = iter_lower_bound.y; iy <= iter_upper_bound.y; iy++) {
            ...
            if (in_bounds(column_pos, Int2(visible_center.x - vld.radius, visible_center.y - vld.radius), Int2(visible_center.x + vld.radius + 1, visible_center.y + vld.radius + 1))) {
                ...
                int wi = vl.input_cis[visible_column_index] + vld.size.z * (offset.y + diam * (offset.x + diam * hidden_cell_index_max));
    ...
    float act = (sum / 127.0f) / max(1, count) * params.scale;

    vl.recon_acts[visible_column_index] = expf(min(0.0f, act));
}
...
void Encoder::learn(
    ...
    for (int vc = 0; vc < vld.size.z; vc++) {
        ...
        if (vl.recon_acts[visible_cell_index] > max_activation || max_index == -1) {
            max_activation = vl.recon_acts[visible_cell_index];
            max_index = vc;
        }
    }

    if (max_index == vl.input_cis[visible_column_index])
        return;

    for (int vc = 0; vc < vld.size.z; vc++) {
        int visible_cell_index = vc + visible_cells_start;

        int delta = roundf(params.lr * 127.0f * ((vc == vl.input_cis[visible_column_index]) - expf(vl.recon_acts[visible_cell_index])));
...
void Encoder::learn(
    ...
    for (int vli = 0; vli < visible_layers.size(); vli++) {
        ...
        #pragma omp parallel for
        for (int i = 0; i < num_visible_columns; i++)
            learn(Int2(i / vld.size.y, i % vld.size.y), vli, params);
    }
}
...
int Encoder::size() const {
    ...
    for (int vli = 0; vli < visible_layers.size(); vli++) {
        ...
        size += sizeof(Visible_Layer_Desc) + vl.weights.size() * sizeof(S_Byte) + 2 * vl.input_cis.size() * sizeof(int) + sizeof(float);
    }
