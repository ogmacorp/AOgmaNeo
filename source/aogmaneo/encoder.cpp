...
void Encoder::backward(
    ...
    vl.recons_match[visible_column_index] = recon_match / max(1, count);
}
        ...
        for (int ix = iter_lower_bound.x; ix <= iter_upper_bound.x; ix++)
            for (int iy = iter_lower_bound.y; iy <= iter_upper_bound.y; iy++) {
                ...
                for (int vc = 0; vc < vld.size.z; vc++) {
                    ...
                    if (vc == in_ci) {
                        Byte recon_match = vl.recons_match[visible_column_index];

                        vl.weights_match[wi] = min(255, vl.weights_match[wi] + 255 - recon_match);
                    }
...
void Encoder::init_random(
    ...
    for (int vli = 0; vli < visible_layers.size(); vli++) {
        ...
        vl.recons_match.resize(num_visible_columns);
    }
