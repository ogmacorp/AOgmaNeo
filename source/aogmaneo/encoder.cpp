...
void Encoder::forward(
    ...
    for (int hc = 0; hc < hidden_size.z; hc++) {
        ...
        sum /= max(limit_small, total_importance * count * 255);
    ...
    hidden_comparisons[hidden_column_index] = (max_activation < params.min_activity ? 1.0f + randf(state) : max_activation);

    learn_cis[hidden_column_index] = (max_activation < params.min_activity ? rand(state) % hidden_size.z : max_index);
...
long Encoder::size() const {
    ...
    for (int vli = 0; vli < visible_layers.size(); vli++) {
        ...
        size += sizeof(Visible_Layer_Desc) + vl.weights.size() * sizeof(Byte) + vl.hidden_counts.size() * sizeof(int) + sizeof(float);
    }
