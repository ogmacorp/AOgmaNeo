...
void Encoder::forward(
    ...
    for (int vli = 0; vli < visible_layers.size(); vli++) {
        ...
        count1 += vl.importance * sub_count * vld.size.z;
    ...
    for (int hc = 0; hc < hidden_size.z; hc++) {
        ...
        float match = 1.0f - hidden_sums[hidden_cell_index] / count;

        float activation = hidden_sums[hidden_cell_index] / (params.choice + count1 - hidden_totals[hidden_cell_index]);
...
void Encoder::learn(
    ...
    float rate = (hidden_totals[hidden_cell_index_max] > 0.0f ? params.lr : 1.0f);
