        ...
        for (int ix = iter_lower_bound.x; ix <= iter_upper_bound.x; ix++)
            for (int iy = iter_lower_bound.y; iy <= iter_upper_bound.y; iy++) {
                ...
                for (int hc = 0; hc < hidden_size.z; hc++) {
                    ...
                    int wi = wi_offset + hidden_stride * hidden_cell_index;

                    float in_value = (in_ci + 0.5f) * size_z_inv;

                    float delta = in_value - vl.protos[wi];

                    hidden_acts[hidden_cell_index] -= delta * delta * scale;
                }
            }
...
void Encoder::learn(
    ...
    for (int dhc = -scan_radius; dhc <= scan_radius; dhc++) {
        ...
        float rate = hidden_rates[hidden_cell_index] * (dhc == 0 ? 1.0f : params.falloff) * (num_higher == 0 ? 1.0f : params.falloff);
