        ...
        for (int ix = iter_lower_bound.x; ix <= iter_upper_bound.x; ix++)
            for (int iy = iter_lower_bound.y; iy <= iter_upper_bound.y; iy++) {
                ...
                for (int hc = 0; hc < hidden_size.z; hc++) {
                    ...
                    hidden_acts[hidden_cell_index] += vl.weights[wi] * (1.0f - vl.recon_acts[visible_column_index]) * influence;
                }
            }
    }
...
void Encoder::backward(
    ...
    for (int ix = iter_lower_bound.x; ix <= iter_upper_bound.x; ix++)
        for (int iy = iter_lower_bound.y; iy <= iter_upper_bound.y; iy++) {
            ...
            if (in_bounds(column_pos, Int2(visible_center.x - vld.radius, visible_center.y - vld.radius), Int2(visible_center.x + vld.radius + 1, visible_center.y + vld.radius + 1))) {
                ...
                int wi = in_ci + vld.size.z * (offset.y + diam * (offset.x + diam * hidden_cell_index_max));

                recon_sum += vl.weights[wi];
                count++;
            }
        }
    ...
    vl.recon_acts[visible_column_index] = max(0.0f, 1.0f + min(0, recon_sum - count * 127) * recon_scale);
}
...
void Encoder::learn(
    ...
    for (int vc = 0; vc < vld.size.z; vc++) {
        ...
        vl.recon_sums[visible_cell_index] = rand_roundf(params.lr * 255.0f * ((vc == target_ci) - max(0.0f, 1.0f + min(0, recon_sum - count * 127) * recon_scale)), state);
    }
