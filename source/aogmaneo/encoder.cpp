...
void Encoder::forward(
    ...
    for (int hc = 0; hc < hidden_size.z; hc++) {
        ...
        for (int vli = 0; vli < visible_layers.size(); vli++) {
            ...
            total0 += vl.hidden_totals0[hidden_cell_index] * influence;
        ...
        if ((!hidden_committed_flags[hidden_cell_index] || match0 >= params.vigilance_high) && activation1 > max_activation) {
            max_activation = activation1;
            max_index = hc;
        }

        if (activation1 > max_complete_activation) {
            max_complete_activation = activation1;
            max_complete_index = hc;
        }

        if (!hidden_committed_flags[hidden_cell_index] || match0 >= params.vigilance_low)
            pass_low = true;
    }

    hidden_comparisons[hidden_column_index] = (pass_low ? max_complete_activation : 0.0f);
...
void Encoder::init_random(
    ...
    for (int vli = 0; vli < visible_layers.size(); vli++) {
        ...
        vl.hidden_totals0.resize(num_hidden_cells);
                ...
                for (int ix = iter_lower_bound.x; ix <= iter_upper_bound.x; ix++)
                    for (int iy = iter_lower_bound.y; iy <= iter_upper_bound.y; iy++) {
                        ...
                        for (int vc = 0; vc < vld.size.z; vc++) {
                            ...
                            sub_total0 += vl.weights0[wi];
                            sub_total1 += vl.weights1[wi];
                        }
                    }

                vl.hidden_totals0[hidden_cell_index] = sub_total0;
...
long Encoder::size() const {
    ...
    for (int vli = 0; vli < visible_layers.size(); vli++) {
        ...
        size += sizeof(Visible_Layer_Desc) + 2 * vl.weights0.size() * sizeof(Byte) + 2 * vl.hidden_totals0.size() * sizeof(int) + sizeof(float);
    }
...
void Encoder::write(
    ...
    for (int vli = 0; vli < visible_layers.size(); vli++) {
        ...
        writer.write(&vl.hidden_totals0[0], vl.hidden_totals0.size() * sizeof(int));
...
void Encoder::read(
    ...
    for (int vli = 0; vli < visible_layers.size(); vli++) {
        ...
        reader.read(&vl.hidden_totals0[0], vl.hidden_totals0.size() * sizeof(int));
    ...
    case merge_average:
        for (int vli = 0; vli < visible_layers.size(); vli++) {
            ...
            for (int i = 0; i < vl.weights0.size(); i++) {
                ...
                vl.weights0[i] = roundf2b(total0 / encoders.size());
                vl.weights1[i] = roundf2b(total1 / encoders.size());
            }
        }
