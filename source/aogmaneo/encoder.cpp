...
void Encoder::forward(
        ...
        for (int ix = iter_lower_bound.x; ix <= iter_upper_bound.x; ix++)
            for (int iy = iter_lower_bound.y; iy <= iter_upper_bound.y; iy++) {
                ...
                for (int hc = 0; hc < hidden_size.z; hc++) {
                    ...
                    Byte delta = 255 - vl.weights[wi];

                    vl.hidden_sums[hidden_cell_index] += delta * delta;
                    vl.hidden_sums1[hidden_cell_index] += vl.weights[wi] * vl.weights[wi];
                }
            }
    }
    ...
    for (int hc = 0; hc < hidden_size.z; hc++) {
        ...
        sum1 /= max(limit_small, count);
        total /= max(limit_small, count);

        float activation = -(sum + total - sum1);
        ...
        for (int iy = iter_lower_bound.y; iy <= iter_upper_bound.y; iy++) {
            ...
            if (in_bounds(column_pos, Int2(visible_center.x - vld.radius, visible_center.y - vld.radius), Int2(visible_center.x + vld.radius + 1, visible_center.y + vld.radius + 1))) {
                ...
                for (int vc = 0; vc < vld.size.z; vc++) {
                    ...
                    ATOMIC
                    vl.hidden_totals[hidden_cell_index_max] += vl.weights[wi] * vl.weights[wi] - w_old * w_old;
                }
            }
        }
}
                ...
                for (int ix = iter_lower_bound.x; ix <= iter_upper_bound.x; ix++)
                    for (int iy = iter_lower_bound.y; iy <= iter_upper_bound.y; iy++) {
                        ...
                        for (int vc = 0; vc < vld.size.z; vc++) {
                            ...
                            total += vl.weights[wi] * vl.weights[wi];
                        }
                    }
