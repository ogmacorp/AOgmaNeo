...
void Encoder::learn(
    ...
    for (int vc = 0; vc < vld.size.z; vc++) {
        ...
        vl.recon_deltas[visible_cell_index] = min(0.0f, params.lr * 255.0f * ((vc == target_ci) - expf((static_cast<float>(recon_sum) / (max(1, count) * 255) - 1.0f) * params.scale))) + params.forget;
    }
