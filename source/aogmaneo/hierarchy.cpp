...
void Hierarchy::step(
    ...
    for (int l = decoders.size() - 1; l >= 0; l--) {
        ...
        for (int d = 0; d < decoders[l].size(); d++) {
            if (learn_enabled)
                decoders[l][d].learn((l == 0 ? input_cis[i_indices[d]] : &encoders[l - 1].get_hidden_cis()), (l == 0 ? nullptr : &encoders[l - 1].get_hidden_commits()), (l == 0 ? params.ios[d].decoder : params.layers[l].decoder));
