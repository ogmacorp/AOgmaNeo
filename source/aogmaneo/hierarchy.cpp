...
void Hierarchy::init_random(
    ...
    for (int l = 0; l < layer_descs.size(); l++) {
        ...
        if (l == 0) {
            ...
            for (int i = 0; i < io_sizes.size(); i++) {
                if (io_descs[i].type == action) {
                    ...
                    actors[d_index].init_random(io_sizes[i], 1, io_descs[i].history_capacity, a_visible_layer_descs);
        ...
        if (layer_descs[l].recurrent_radius >= 0) {
            ...
            e_visible_layer_descs[e_visible_layer_descs.size() - 1].is_recurrent = true;
        }
