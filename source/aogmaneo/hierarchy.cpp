        ...
        if (l == 0) {
            ...
            for (int i = 0; i < io_sizes.size(); i++) {
                if (io_descs[i].type == action) {
                    ...
                    actors[d_index].init_random(io_sizes[i], io_descs[i].value_size, io_descs[i].value_num_dendrites_per_cell, io_descs[i].num_dendrites_per_cell, a_visible_layer_descs);
