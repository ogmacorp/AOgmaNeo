    ...
    for (int l = decoders.size() - 1; l >= 0; l--) {
        if (updates[l]) {
            ...
            for (int d = 0; d < decoders[l].size(); d++) {
                if (learn_enabled)
                    decoders[l][d].learn(&histories[l][l == 0 ? i_indices[d] : 0][l == 0 ? 0 : d], (l == 0 ? nullptr : &encoders[l - 1].get_hidden_commits()), (l == 0 ? params.ios[d].decoder : params.layers[l].decoder));

                decoders[l][d].activate(layer_input_cis, (l == 0 ? nullptr : &encoders[l - 1].get_hidden_commits()), (l == 0 ? params.ios[d].decoder : params.layers[l].decoder));
            }

            if (l == 0) {
                for (int d = 0; d < actors.size(); d++)
                    actors[d].step(layer_input_cis, input_cis[i_indices[d + io_sizes.size()]], reward, learn_enabled, mimic, params.ios[d].actor);
            }
        }
    }
}
