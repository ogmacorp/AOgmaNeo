...
void Actor::forward(
    ...
    int targetCIPrev = (*hiddenTargetCIsPrev)[hiddenColumnIndex];

    float valuePrev = hiddenValues[address3(Int3(columnPos.x, columnPos.y, targetCIPrev), hiddenSize)];
        ...
        for (int vli = 0; vli < visibleLayers.size(); vli++) {
            ...
            for (int ix = iterLowerBound.x; ix <= iterUpperBound.x; ix++)
                for (int iy = iterLowerBound.y; iy <= iterUpperBound.y; iy++) {
                    ...
                    float weight = vl.weights[inCI + vld.size.z * (offset.y + diam * (offset.x + diam * hiddenIndex))];
    ...
    if (learnEnabled) {
        float delta = alpha * (reward + gamma * maxActivation - valuePrev);
                ...
                for (int ix = iterLowerBound.x; ix <= iterUpperBound.x; ix++)
                    for (int iy = iterLowerBound.y; iy <= iterUpperBound.y; iy++) {
                        ...
                        for (int vc = 0; vc < vld.size.z; vc++) {
                            ...
                            if (vc == inCI)
                                vl.traces[wi] = (hc == targetCIPrev ? 1.0f : 0.0f);
                            else
