                ...
                for (int iy = iterLowerBound.y; iy <= iterUpperBound.y; iy++) {
                    ...
                    for (int vc = 0; vc < vld.size.z; vc++) {
                        ...
                        if (vc == inCIPrev && hc == targetCI)
                            vl.traces[wi] = vl.traces[wi] * traceDecay + expf(-vl.traces[wi] * traceTemp);
                        else
...
void Actor::write(
    ...
    writer.write(reinterpret_cast<const void*>(&traceTemp), sizeof(float));
...
void Actor::read(
    ...
    reader.read(reinterpret_cast<void*>(&traceTemp), sizeof(float));
