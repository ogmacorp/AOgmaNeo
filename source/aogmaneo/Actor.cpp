...
void Actor::learn(
    ...
    for (int zi = 0; zi < supportSize; zi++) {
        ...
        // Distribute
        hiddenTargetProbs[address3(Int3(pos.x, pos.y, lower), Int3(hiddenSize.x, hiddenSize.y, supportSize))] += hiddenProbs[zi] * (upper - b);
        hiddenTargetProbs[address3(Int3(pos.x, pos.y, upper), Int3(hiddenSize.x, hiddenSize.y, supportSize))] += hiddenProbs[zi] * (b - lower);
    }
        ...
        for (int vli = 0; vli < visibleLayers.size(); vli++) {
            ...
            for (int ix = iterLowerBound.x; ix <= iterUpperBound.x; ix++)
                for (int iy = iterLowerBound.y; iy <= iterUpperBound.y; iy++) {
                    ...
                    vl.valueWeights[inC + vld.size.z * (offset.y + diam * (offset.x + diam * hiddenIndex))] += delta;
                }
        }
    }
    ...
    float deltaAction = beta * ((newValue - value) - sum);
...
void Actor::initRandom(
    ...
    for (int vli = 0; vli < visibleLayers.size(); vli++) {
        ...
        vl.valueWeights.resize(numHiddenColumns * supportSize * area * vld.size.z);
