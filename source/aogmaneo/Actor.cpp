...
void Actor::learn(
    ...
    float deltaValue = alpha * tdError;
    ...
    for (int hc = 0; hc < hiddenSize.z; hc++) {
        ...
        float deltaAction = (mimic || tdError > 0.0f ? beta : -beta) * ((hc == targetCI ? 1.0f : 0.0f) - hiddenActivations[hiddenCellIndex] / max(0.0001f, total));
