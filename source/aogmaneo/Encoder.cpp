...
void Encoder::forward(
    ...
    if (learnEnabled) {
        for (int hc = 0; hc < hiddenSize.z; hc++) {
            ...
            float delta = elr * (*hiddenErrors)[hiddenColumnIndex] * ((hc == hiddenCIs[hiddenColumnIndex] ? 1.0f : 0.0f) - hiddenActivations[hiddenCellIndex]);
...
void Encoder::align(
    ...
    for (int hc = 0; hc < hiddenSize.z; hc++) {
        ...
        float delta = alr * ((hc == (*hiddenTargetCIs)[hiddenColumnIndex] ? 1.0f : 0.0f) - hiddenActivations[hiddenCellIndex]);
...
void Encoder::step(
    ...
    if (learnEnabled && hiddenTargetCIs != nullptr) {
        #pragma omp parallel for
        for (int i = 0; i < numHiddenColumns; i++)
            align(Int2(i / hiddenSize.y, i % hiddenSize.y), hiddenTargetCIs);
    }
}
