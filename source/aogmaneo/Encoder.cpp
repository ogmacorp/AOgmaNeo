...
void Encoder::forward(
    ...
    if (it == explainIters - 1 && learnEnabled) {
...
void Encoder::step(
    ...
    for (int it = 0; it < explainIters; it++) {
        ...
        for (int i = 0; i < numHiddenColumns; i++)
            forward(Int2(i / hiddenSize.y, i % hiddenSize.y), inputCIs, hiddenErrors, it, learnEnabled);
...
int Encoder::size() const {
    ...
    for (int vli = 0; vli < visibleLayers.size(); vli++) {
        ...
        size += sizeof(VisibleLayerDesc) + vl.weights.size() * sizeof(float) + vl.rates.size() * sizeof(float) + vl.inputCIsPrev.size() * sizeof(int) + sizeof(float);
    }
