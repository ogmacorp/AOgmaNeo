...
void Encoder::forward(
    ...
    if (learnEnabled) {
        ...
        float strength = sigmoid((*hiddenTargets)[hiddenColumnIndex]);
            ...
            for (int ix = iterLowerBound.x; ix <= iterUpperBound.x; ix++)
                for (int iy = iterLowerBound.y; iy <= iterUpperBound.y; iy++) {
                    ...
                    for (int vc = 0; vc < vld.size.z; vc++) {
                        ...
                        vl.weights[wi] += lr * ((vc == inCIPrev) * strength - vl.weights[wi]);
                    }
                }
        }
    }
...
void Encoder::step(
    ...
    for (int i = 0; i < numHiddenColumns; i++)
        forward(Int2(i / hiddenSize.y, i % hiddenSize.y), inputCIs, hiddenTargets, learnEnabled);
