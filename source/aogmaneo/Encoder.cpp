    ...
    for (int hc = 0; hc < hiddenSize.z; hc++) {
        ...
        for (int ix = iterLowerBound.x; ix <= iterUpperBound.x; ix++)
            for (int iy = iterLowerBound.y; iy <= iterUpperBound.y; iy++) {
                ...
                if (otherHiddenColumnIndex == hiddenColumnIndex) // No self-connection
                    continue;
        ...
        hiddenActivations[hiddenCellIndex] += hiddenStimuli[hiddenCellIndex] - sum;
        ...
        for (int ix = iterLowerBound.x; ix <= iterUpperBound.x; ix++)
            for (int iy = iterLowerBound.y; iy <= iterUpperBound.y; iy++) {
                ...
                for (int vc = 0; vc < vld.size.z; vc++) {
                    ...
                    vl.weights[wi] = min(1.0f, max(0.0f, vl.weights[wi] + hiddenRates[hiddenCellIndexMax] * ((vc == inCI) - 1.0f / vld.size.z)));
                }
            }
    }
    ...
    for (int ix = iterLowerBound.x; ix <= iterUpperBound.x; ix++)
        for (int iy = iterLowerBound.y; iy <= iterUpperBound.y; iy++) {
            ...
            for (int ohc = 0; ohc < hiddenSize.z; ohc++) {
                ...
                laterals[wi] = max(0.0f, laterals[wi] + hiddenRates[hiddenCellIndexMax] * ((ohc == inCI) - 1.0f / hiddenSize.z));
            }
        }
