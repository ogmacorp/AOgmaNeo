...
void Encoder::forward(
    ...
    if (learnEnabled) {
        for (int hc = 0; hc < hiddenSize.z; hc++) {
            ...
            float delta = wlr * (*hiddenErrors)[hiddenColumnIndex] * ((hc == hiddenCIs[hiddenColumnIndex]) - hiddenActivations[hiddenCellIndex]);
    ...
    for (int hc = 0; hc < hiddenSize.z; hc++) {
        ...
        float biasedActivation = sum + hiddenBiases[hiddenCellIndex];

        if (learnEnabled)
            hiddenBiases[hiddenCellIndex] -= blr * biasedActivation;

        if (biasedActivation > maxActivation || maxIndex == -1) {
            maxActivation = biasedActivation;
            maxIndex = hc;
        }
    }
...
void Encoder::write(
    ...
    writer.write(reinterpret_cast<const void*>(&wlr), sizeof(float));
...
void Encoder::read(
    ...
    reader.read(reinterpret_cast<void*>(&wlr), sizeof(float));
