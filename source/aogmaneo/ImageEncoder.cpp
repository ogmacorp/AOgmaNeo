...
void ImageEncoder::forward(
    ...
    if (learnEnabled) {
        for (int hc = 0; hc < hiddenSize.z; hc++) {
            ...
            float strength = min(1.0f, sqrtf(-maxActivation) * scale) * expf(-falloff * diff * diff / max(0.0001f, hiddenRates[hiddenCellIndex])) * hiddenRates[hiddenCellIndex];
...
int ImageEncoder::size() const {
    int size = sizeof(Int3) + 3 * sizeof(float) + hiddenCIs.size() * sizeof(int) + hiddenRates.size() * sizeof(float) + sizeof(int);
