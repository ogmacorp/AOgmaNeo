    ...
    for (int l = dLayers.size() - 1; l >= 0; l--) {
        ...
        if (l == 0) {
            for (int d = 0; d < aLayers.size(); d++)
                aLayers[d].step(dInputCIs, inputCIs[iIndices[d + ioSizes.size()]], reward, learnEnabled);
        }
    }
}

int Hierarchy::size() const {
    int size = 4 * sizeof(int) + ioSizes.size() * sizeof(Int3) + ioSizes.size() * sizeof(Byte) + iIndices.size() * sizeof(int) + dIndices.size() * sizeof(int);
