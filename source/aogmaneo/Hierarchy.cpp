        ...
        if (l == 0) {
            ...
            for (int i = 0; i < ioSizes.size(); i++) {
                if (ioDescs[i].type == prediction) {
                    ...
                    dLayers[l][dIndex].initRandom(ioSizes[i], ioDescs[i].numDendrites, ioDescs[i].historyCapacity, dVisibleLayerDesc);
            ...
            for (int t = 0; t < layerDescs[l].ticksPerUpdate; t++)
                dLayers[l][t].initRandom(layerDescs[l - 1].hiddenSize, layerDescs[l].numDendrites, layerDescs[l].historyCapacity, dVisibleLayerDesc);
        }
