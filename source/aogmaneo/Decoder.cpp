...
void Decoder::learn(
    ...
    for (int it = 0; it < pruneIters; it++) {
        int hc1 = rand(state) % hiddenSize.z;
        int hc2 = (hc1 + 1 + rand(state) % (hiddenSize.z - 1)) % hiddenSize.z;
        
        int hiddenCellIndex1 = hc1 + hiddenCellsStart;
        int hiddenCellIndex2 = hc2 + hiddenCellsStart;
        ...
        for (int vli = 0; vli < visibleLayers.size(); vli++) {
            ...
            for (int ix = iterLowerBound.x; ix <= iterUpperBound.x; ix++)
                for (int iy = iterLowerBound.y; iy <= iterUpperBound.y; iy++) {
                    ...
                    int wiStart1 = vld.size.z * (offset.y + diam * (offset.x + diam * hiddenCellIndex1));
                    int wiStart2 = vld.size.z * (offset.y + diam * (offset.x + diam * hiddenCellIndex2));

                    for (int vc = 0; vc < vld.size.z; vc++) {
                        int wi1 = vc + wiStart1;
                        int wi2 = vc + wiStart2;

                        d += vl.weights[wi1] * vl.weights[wi2];
                        n += vl.weights[wi2] * vl.weights[wi2];
                    }
                }
        }

        float proj = pr * d / max(0.0001f, n);
            ...
            for (int ix = iterLowerBound.x; ix <= iterUpperBound.x; ix++)
                for (int iy = iterLowerBound.y; iy <= iterUpperBound.y; iy++) {
                    ...
                    for (int vc = 0; vc < vld.size.z; vc++) {
                        ...
                        vl.weights[wi1] -= vl.weights[wi2] * proj;
                    }
                }
        }
    }
