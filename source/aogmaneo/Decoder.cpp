        ...
        for (int ix = iterLowerBound.x; ix <= iterUpperBound.x; ix++)
            for (int iy = iterLowerBound.y; iy <= iterUpperBound.y; iy++) {
...

                {
                    ...
                    vl.weights[wi] = max(-127, static_cast<int>(vl.weights[wi]) - 1);
                }

                {
                    int wi = inCIPrev + vld.size.z * (offset.y + diam * (offset.x + diam * hiddenCellIndexTarget));

                    vl.weights[wi] = min(127, static_cast<int>(vl.weights[wi]) + 1);
                }
            }
    }
}
...
void Decoder::initRandom(
    ...
    for (int vli = 0; vli < visibleLayers.size(); vli++) {
        ...
        vl.weights = SByteBuffer(numHiddenCells * area * vld.size.z, 0); // Ceil;
