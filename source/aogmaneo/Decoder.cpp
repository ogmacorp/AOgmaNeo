        ...
        for (int vli = 0; vli < visibleLayers.size(); vli++) {
            ...
            for (int ix = iterLowerBound.x; ix <= iterUpperBound.x; ix++)
                for (int iy = iterLowerBound.y; iy <= iterUpperBound.y; iy++) {
                    ...
                    vl.weights[wi] = min(127, max(-127, vl.weights[wi] + static_cast<int>(roundf(delta * vl.gates[visibleColumnIndex]))));
                }
        }
    }
...
int Decoder::size() const {
    int size = sizeof(Int3) + 2 * sizeof(float) + sizeof(int) + hiddenActs.size() * sizeof(float) + hiddenCIs.size() * sizeof(int) + sizeof(int);
