...
void Decoder::fLearn(
    ...
    for (int hc = 0; hc < hiddenSize.z; hc++) {
        ...
        float delta = flr * ((hc == targetCI) - hiddenActs[hiddenCellIndex]);
        ...
        for (int ix = iterLowerBound.x; ix <= iterUpperBound.x; ix++)
            for (int iy = iterLowerBound.y; iy <= iterUpperBound.y; iy++) {
                ...
                if (inBounds(columnPos, Int2(visibleCenter.x - vld.radius, visibleCenter.y - vld.radius), Int2(visibleCenter.x + vld.radius + 1, visibleCenter.y + vld.radius + 1))) {
                    ...
                    int wi = vc + vld.size.z * (offset.y + diam * (offset.x + diam * hiddenCellIndexTarget));
...
void Decoder::learn(
    ...
    #pragma omp parallel for
    for (int i = 0; i < numHiddenColumns; i++)
        fLearn(Int2(i / hiddenSize.y, i % hiddenSize.y), hiddenTargetCIs);
    ...
    for (int vli = 0; vli < visibleLayers.size(); vli++) {
        ...
        for (int i = 0; i < numVisibleColumns; i++)
            bLearn(Int2(i / vld.size.y, i % vld.size.y), hiddenTargetCIs, vli);
    }
}
...
int Decoder::size() const {
    ...
    for (int vli = 0; vli < visibleLayers.size(); vli++) {
        ...
        size += sizeof(VisibleLayerDesc) + 2 * vl.fWeights.size() * sizeof(float) + vl.inputCIsPrev.size() * sizeof(int);
    }
...
void Decoder::write(
    ...
    writer.write(reinterpret_cast<const void*>(&flr), sizeof(float));
    writer.write(reinterpret_cast<const void*>(&blr), sizeof(float));
...
void Decoder::read(
    ...
    reader.read(reinterpret_cast<void*>(&flr), sizeof(float));
    reader.read(reinterpret_cast<void*>(&blr), sizeof(float));
