        ...
        for (int vli = 0; vli < visibleLayers.size(); vli++) {
            ...
            for (int ix = iterLowerBound.x; ix <= iterUpperBound.x; ix++)
                for (int iy = iterLowerBound.y; iy <= iterUpperBound.y; iy++) {
                    ...
                    sum += weightLookup(vl.weights[wi] * byteInv);
                }
        }
        ...
        for (int ix = iterLowerBound.x; ix <= iterUpperBound.x; ix++)
            for (int iy = iterLowerBound.y; iy <= iterUpperBound.y; iy++) {
                ...
                for (int vc = 0; vc < vld.size.z; vc++) {
                    ...
                    vl.weights[wi] = min(255, max(0, vl.weights[wi] + roundf(lr * 255.0f * ((vc == inCIPrev) - visibleSizeZInv))));
                }
            }
    }
}
...
int Decoder::size() const {
    int size = sizeof(Int3) + sizeof(int) + 2 * sizeof(float) + hiddenActivations.size() * sizeof(float) + hiddenCIs.size() * sizeof(int) + sizeof(int);
